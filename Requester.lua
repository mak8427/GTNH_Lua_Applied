...
local function checkCrafting()
  ...
  for fullItemName, data in pairs(watchitems) do
    ...
    print("\n----------------------------------------")
    print(string.format("[DEBUG] Checking item: %s | Desired count: %d | Batch size: %d", fullItemName, desiredCount, batchSize))
    ...
    if desiredCount > currentStock then
      ...
      print(string.format("[DEBUG] Need to craft %d more (difference: %d) of %s (Label: %s)", reqsize, difference, itemname, label))
      ...
      if recipe then
        ...
        print(string.format("[DEBUG] Craft initiated at %s on CPU %s for %s", time_format(monitors[fullItemName].startTime), tostring(freeCPU), label))
      else
    ...
    os.sleep(0.5)  -- Short delay between requests
  end
  return monitors
end
    ...
    for itemKey, data in pairs(monitors) do
      ...
      if unfinished[itemKey] then
        ...
        elseif monitor.isDone() then
          print(string.format("[DEBUG] Crafting of %s  CPU: %s completed successfully after %d seconds! Produced: %d, Remaining: %d",
                data.label,tostring(data.cpuNum), elapsed, produced, remaining))
          unfinished[itemKey] = nil
        else
          print(string.format("[DEBUG] Crafting in progress for %s  CPU: %s ... Started at %s, Elapsed: %d seconds, Produced: %d, Remaining: %d",
