    ...
    for itemKey, data in pairs(monitors) do
      ...
      if unfinished[itemKey] then
        ...
        elseif monitor.isDone() then
          print(string.format("[DEBUG] Crafting of %s (Label: %s, CPU: %s) completed successfully after %d seconds! Produced: %d, Remaining: %d",
          ...
          print(string.format("[DEBUG] Crafting in progress for %s (Label: %s, CPU: %s)... Started at %s, Elapsed: %d seconds, Produced: %d, Remaining: %d",
                data.label, data.label, tostring(data.cpuNum), time_format(startTime), elapsed, produced, remaining))
        end
      end
    end
    os.sleep(5)
